...
**tg-re-proxy** (Telegram Redirect Proxy) — универсальный прозрачный прокси-сервер для перехвата и обхода блокировок Telegram на уровне сетевого шлюза без настройки клиентских устройств.

Проект разработан как автономный скрипт с минимальным потреблением ресурсов и может быть развернут на **Raspberry Pi (или любом одноплатном ПК)**, непосредственно на **совместимом роутере** (OpenWRT, Keenetic, MikroTik) или на **персональном компьютере (Linux)** для перенаправления локального или раздаваемого трафика. Форк оригинального [tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy), адаптированный для работы в режиме Transparent Proxy (через перенаправление `iptables`/`ufw`).
