...
* **Невозможность установки на iOS/iPadOS:** В отличие от Android (через Termux) или ПК, на iOS из-за ограничений системы невозможно запустить серверную часть прокси локально на устройстве. Единственный вариант — ставить прокси на внешнее устройство (например, Raspberry Pi).
* **UX-катастрофа при выходе из дома:** Из-за необходимости держать прокси на внешнем сервере (Pi), прописанный в Telegram локальный IP-адрес (например, `192.168.0.24`) работает только в домашней сети. При выходе из дома и переключении на мобильную сеть (LTE/5G) локальный прокси становится недоступен, Telegram зависает на подключении, и прокси приходится отключать вручную.
* **Проблемы фонового режима на iOS:** Telegram в фоновом режиме на iOS часто некорректно работает с настроенным вручную прокси, что приводит к задержкам push-уведомлений и долгому переподключению при открытии приложения.
...
* **Zero-config на клиенте:** Прокси-сервер в Telegram на устройствах полностью выключен. Дома трафик перехватывается шлюзом автоматически, а вне дома (LTE/5G) устройство работает напрямую без изменения настроек.
