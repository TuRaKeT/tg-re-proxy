...
1. **Неудобство настройки:** Требуется ручная конфигурация каждого нового устройства в домашней сети.
2. **Проблемы на мобильных устройствах (iOS/iPadOS):** На iPhone и iPad клиенты Telegram в фоновом режиме часто некорректно работают с настроенными вручную прокси. Это приводит к задержкам получения push-уведомлений, долгой загрузке при открытии приложения и зависанию в статусе **«Обновление» (Updating)**.
3. **Обнаружение трафика:** Ручные MTProto-прокси легко обнаруживаются провайдерами по характерным сигнатурам подключения.
...
1. **Нулевая настройка клиентов (Zero-config):** Мобильные телефоны (iPhone, Android), планшеты (iPad) и компьютеры подключаются к домашнему Wi-Fi и работают с Telegram напрямую. Им не требуется никаких настроек прокси внутри клиента.
2. **Стабильная и быстрая работа на iOS (iPhone/iPad):** 
   - Поскольку прокси работает незаметно на уровне шлюза, операционная система iOS считает это прямым подключением. Telegram на iPhone/iPad мгновенно переходит в статус «Подключено», не висит на «Обновлении» и стабильно получает фоновые push-уведомления.
...
3. **Монолитность и легковесность:** 
   ...
   - Для работы не требуются сторонние тяжелые библиотеки и фреймворки (скрипт умеет работать напрямую через встроенные библиотеки и системный OpenSSL).
